...
#python ppo_share_copy.py --gym-id BipedalWalkerHardcore-v3
...
def parse_args():
    ...
    parser.add_argument("--gym-id", type=str, default="BipedalWalkerHardcore-v3",
...
def compute_advantage(agent,buffer, args, next_obs, next_done):
    # 计算优势
    with torch.no_grad():
        next_value = agent.get_value(next_obs).T.reshape(-1)
        if args.gae:
            lastgaelam = 0
...
class PPOAgent(nn.Module):
    def __init__(self, obs_space, action_space,args) -> None:
        ...
        self.actor_mean = nn.Sequential(
            ...
            nn.ReLU(),
            layer_init(nn.Linear(64, np.array(action_space.shape).prod()), std=0.01)
        )
        ...
        self.optimizer = torch.optim.Adam([
            {'params': self.actor_mean.parameters(), 'lr': args.learning_rate},
            {'params': self.actor_logstd, 'lr': args.learning_rate},
...
if __name__ == "__main__":
    ...
    # env = gym.wrappers.RecordEpisodeStatistics(env)
    onnx_dir = 'onnx_module'
    if not os.path.exists(onnx_dir):
        os.makedirs(onnx_dir)

    # 进程并行
    #envs=gym.vector.AsyncVectorEnv([make_env(args.gym_id,args.seed+i,i,args.capture_video,run_name) for i in range(args.num_envs) ])
    # 顺序执行num_envs个环境
    ...
    buffer = RolloutBuffer(envs.single_observation_space, envs.single_action_space, args)
    agent = PPOAgent(envs.single_observation_space, envs.single_action_space,args).to(args.device)
        ...
        for step in range(0,args.num_steps):
            ...
            for info in infos:
                if "episode" in info.keys():
                    if mx_rs<info['episode']['r'] and info['episode']['r']>300:
                        mx_rs=info['episode']['r']
                        timestamp = datetime.datetime.now().strftime('%Y_%m_%d_%H_%M')
                        save_path = f"BipedalWalker-v3/{timestamp}_{args.gym_id}.pth"
    ...
    # 训练结束后导出 ONNX 模型，只导出actor网络
    actor_network = agent.actor_mean
    dummy_input = torch.randn(1, *envs.single_observation_space.shape).to(args.device)
    onnx_path = os.path.join(onnx_dir, 'ppo_actor2.onnx')
    torch.onnx.export(
        actor_network,
        dummy_input,
        onnx_path,
