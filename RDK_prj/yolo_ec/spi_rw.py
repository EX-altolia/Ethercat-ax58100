...
def writeinputdata(spi):
    ...
    for i in range(ec_def.nPdInputSize):
        ...
        if i == 0:
            # 获取ID数据，若无则用默认值[2,7](@ref)
            data = data_timely.result_ID.get(0, default_id)
        elif i == 1:
            # 低8位数据获取（包含默认值）
            raw_x = data_timely.result_Position_x.get(0, default_pos)
            data = raw_x & 0xFF
        elif i == 2:
            # 高8位数据获取（包含默认值）
            raw_x = data_timely.result_Position_x.get(0, default_pos)
            ...
            # 低8位数据获取（包含默认值）
            raw_y = data_timely.result_Position_y.get(0, default_pos)
            data = raw_y & 0xFF
        elif i == 4:
            # 低8位数据获取（包含默认值）
            raw_y = 0x12
